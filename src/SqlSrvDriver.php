...
class SqlSrvDriver implements DriverInterface
{
    ...
    {
        $this->compiler = new SqlSrvCompiler();
        $this->translator = null;
    }

    public function connect(ConnectParams $params)
    {
        $dsn = "sqlsrv:Server={$params->getHost()}";
        if (null !== $params->getPort()) {
            $dsn.= ",{$params->getPort()}";
        }
        $dsn.= ";Database={$params->getDatabase()}";
    ...
    public function executeSelect(CompiledQuery $query): SelectResult
    {
        ...
        if (!$query->getObject() instanceof SelectQuery) {
            throw new InvalidArgumentException('Not a valid SelectQuery');
        }
